...
# To simulate longitudinal data, we start with a 'cross-sectional' data set and 
# convert it to a time-dependent data set. The original cross-sectional data set
# may or may not include time-dependent data in the columns. In the next example,
# we measure outcome `Y` once before and twice after intervention `T` in 
# a randomized trial:
...
# It is also possible to generate longitudinal data with varying numbers of 
# measurement periods as well as varying time intervals between each measurement
#  period. This is done by defining specific variables in the data set that 
#  define the number of observations per subject and the average interval time
#   between each observation. `nCount` defines the number of measurements for
#    an individual; `mInterval` specifies the average time between intervals for 
#    a subject; and `vInterval` specifies the variance of those interval times.
#     If `vInterval` is set to 0 or is not defined, the interval for a subject 
#     is determined entirely by the mean interval. If `vInterval` is greater
#     than 0, time intervals are generated using a gamma distribution with mean 
#     and dispersion specified.

# In this simple example, the cross-sectional data generates individuals with 
# a different number of measurement observations and different times between 
# each observation. Data for two of these individuals is printed:
...
# a call to `addPeriods`. Notice that no parameters need to be set since all 
# information resides in the data set itself:
